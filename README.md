...
| `GEOSITE_BYPASS`   | -                                   | A geosite used to bypass the `ROUTE_FINAL` rule, routing traffic in the opposite direction. You can specify either direct links to `srs` rule files (in the format `http://...srs` or `https://...srs), or file names from the list (without the `geosite-`prefix and **without the extension**) available at: https://github.com/SagerNet/sing-geosite/tree/rule-set Example: if the repository contains`geosite-google.srs`, specify `google` as the value. Note: the lists are updated automatically once per day. |
| `GEOIP_BYPASS`     | -                                   | Same as `GEOSITE_BYPASS`, but for IP-based routing. Specify the file names (without the `geoip-` prefix and **without the extension**) from the rule set in the repository: https://github.com/SagerNet/sing-geoip/tree/rule-set Example: if the repository contains `geoip-cn.srs`, specify `cn`.                                                                                                                                                                                                                    |
| `FINAL_DOMAIN`     | -                                   | A comma-separated list of domain names or suffixes for strict routing according to the route specified in `ROUTE_FINAL`. Traffic to domains matching these entries will be routed strictly following `ROUTE_FINAL`, regardless of `GEOSITE_BYPASS` or `GEOIP_BYPASS` rules. Note: an entry can be a suffix (`example.org`), which allows matching subdomains (`sub.example.org`, etc.). A leading dot is not used for suffixes.                                                                                       |
