...
| `GEOSITE_BYPASS` | A geosite used to bypass the `ROUTE_FINAL` rule, routing traffic in the opposite direction. |

Use file names from the list (without the `geosite-` prefix and **without the extension**) available at:
https://github.com/SagerNet/sing-geosite/tree/rule-set Example: if the repository contains `geosite-google.srs`, specify `google` as the value. | |
| `GEOIP_BYPASS` | - | Same as `GEOSITE_BYPASS`, but for IP-based routing. Specify the file names (without the `geoip-` prefix and **without the extension**) from the rule set in the repository:
https://github.com/SagerNet/sing-geoip/tree/rule-set Example: if the repository contains `geoip-cn.srs`, specify `cn`.|
