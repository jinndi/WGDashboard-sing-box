...
> Parameter–value pairs enclosed in parentheses followed by a question mark `(parameter=value)?` are optional, but if you specify them, their values—if they are not enclosed in angle brackets `<>`— must be exactly as shown in the examples.
> The `sni` parameter — if not specified, it will be set to the host address, provided that it is a domain name.
...
| [`Shadowsocks-2022`](https://sing-box.sagernet.org/configuration/inbound/shadowsocks/)<br />[`multiplex (optional)`](https://sing-box.sagernet.org/configuration/shared/multiplex/)                                                                             | `ss://<base64-encoded(<method>:<password>(:<user_password>)?>@<host>:<port>(/?type=tcp&multiplex=<protocol>)?#myserv` (SIP002)                                  |
...
| [`Hysteria2`](https://sing-box.sagernet.org/configuration/outbound/hysteria2/)                                                                                                                                                                                  | `hysteria2://<password>@<host>:<port>(/?security=tls&sni=<cert-domain>&alpn=h3&insecure=0)?#myserv`                                                                                                                                             |
| [`TUIC`](https://sing-box.sagernet.org/configuration/outbound/tuic/)                                                                                                                                                                                            | `tuic://<UUID>:<password>@<host>:<port>/?(security=tls&sni=<cert-domain>&alpn=h3&insecure=0&congestion_control=<type>&udp_relay_mode=<type>)?#myserv`                                                                                                |
