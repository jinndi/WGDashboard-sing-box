...
| `DOMAIN_PASS`      | -                                   | A comma-separated list of domain names or suffixes for strict routing according to the route specified in `ROUTE_FINAL`. Traffic to domains matching these entries will be routed strictly following `ROUTE_FINAL`, regardless of `GEOSITE_BYPASS` or `GEOIP_BYPASS` rules. Note: an entry can be a suffix (`example.org`), which allows matching subdomains (`sub.example.org`, etc.). A leading dot is not used for suffixes.                                                                                                                                           |
